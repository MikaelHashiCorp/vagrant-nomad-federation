...
cp -ap /vagrant/conf/usa-server-3-consul.hcl /etc/consul.d/
...
for i in {1..30}; do
  if curl -s -f http://192.168.56.83:8500/v1/status/leader > /dev/null 2>&1; then
...
cp -ap /vagrant/conf/usa-server-3-nomad.hcl /etc/nomad.d/
...
for i in {1..30}; do
  if curl -s -f http://192.168.56.83:4646/v1/status/leader > /dev/null 2>&1; then
...
cp -ap /vagrant/conf/usa-server-3-env.sh /etc/profile.d/
